...
export default function LikesList({ postId, onClose }) {
  const navigate = useNavigate()

  const { data: likes = [], isLoading: loading, error } = useQuery({
    queryKey: ['postLikes', postId],
    queryFn: async () => {
      const res = await api(`/api/posts/${encodeURIComponent(postId)}/likes`)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative bg-zinc-900 rounded-xl w-full max-w-sm max-h-[70vh] flex flex-col">
        <div className="flex items-center justify-center px-5 py-4 border-b border-zinc-800 relative">
          <h2 className="text-zinc-100 font-medium">Likes</h2>
        ...
        <div className="overflow-y-auto p-2 flex-1">
          {loading ? (
            ...
            <p className="text-zinc-500 text-sm text-center py-8">Aún no tiene likes</p>
          ) : (
